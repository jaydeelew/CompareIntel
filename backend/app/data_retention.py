...
def cleanup_old_usage_logs(
    ...
    old_logs_query = db.query(UsageLog).filter(
    ...
    for log in old_logs:
        ...
        if log.models_used:
            try:
                models = json.loads(log.models_used) if isinstance(log.models_used, str) else log.models_used
    ...
    if not dry_run:
        for key, data in monthly_data.items():
            ...
            # Calculate averages
            avg_input = Decimal(stats["total_input_tokens"]) / total_comparisons if total_comparisons > 0 else Decimal(0)
            avg_output = Decimal(stats["total_output_tokens"]) / total_comparisons if total_comparisons > 0 else Decimal(0)
            avg_output_ratio = avg_output / avg_input if avg_input > 0 else Decimal(0)
            avg_credits = stats["total_credits_used"] / total_comparisons if total_comparisons > 0 else Decimal(0)
            ...
            for model_id, model_stats in stats["model_breakdown"].items():
                if model_stats["count"] > 0:
                    model_breakdown_json[model_id] = {
                        ...
                        "avg_input_tokens": float(model_stats["total_input"] / model_stats["count"]),
                        "avg_output_tokens": float(model_stats["total_output"] / model_stats["count"]),
                        "total_credits": float(model_stats["total_credits"])
                    }
            
            # Check if aggregate already exists
            existing = db.query(UsageLogMonthlyAggregate).filter(
                and_(
            ...
            if existing:
                ...
                existing.avg_input_tokens = Decimal(existing.total_input_tokens) / existing.total_comparisons if existing.total_comparisons > 0 else Decimal(0)
                existing.avg_output_tokens = Decimal(existing.total_output_tokens) / existing.total_comparisons if existing.total_comparisons > 0 else Decimal(0)
                existing.avg_output_ratio = existing.avg_output_tokens / existing.avg_input_tokens if existing.avg_input_tokens > 0 else Decimal(0)
                existing.avg_credits_per_comparison = existing.total_credits_used / existing.total_comparisons if existing.total_comparisons > 0 else Decimal(0)
                
                # Merge model breakdowns
                existing_breakdown = json.loads(existing.model_breakdown) if existing.model_breakdown else {}
                for model_id, model_stats in model_breakdown_json.items():
                    if model_id in existing_breakdown:
                        ...
                        existing_breakdown[model_id]["total_credits"] += model_stats["total_credits"]
                        # Recalculate averages
                        existing_breakdown[model_id]["avg_input_tokens"] = (
                            (existing_breakdown[model_id]["avg_input_tokens"] * (existing_breakdown[model_id]["count"] - model_stats["count"]) +
                             model_stats["avg_input_tokens"] * model_stats["count"]) / existing_breakdown[model_id]["count"]
                        )
                        existing_breakdown[model_id]["avg_output_tokens"] = (
                            (existing_breakdown[model_id]["avg_output_tokens"] * (existing_breakdown[model_id]["count"] - model_stats["count"]) +
                             model_stats["avg_output_tokens"] * model_stats["count"]) / existing_breakdown[model_id]["count"]
                        )
                    else:
                ...
                aggregate = UsageLogMonthlyAggregate(
                    ...
                    model_breakdown=json.dumps(model_breakdown_json) if model_breakdown_json else None
                )
                db.add(aggregate)
