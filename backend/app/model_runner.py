...
WEB_SEARCH_TOOL = {
    ...
    "function": {
        "name": "search_web",
        "description": "Search the internet for current, real-time information. You MUST use this tool for any question requiring up-to-date information such as current weather, recent news, current events, live data, stock prices, sports scores, or any information that changes over time. Do not provide current information without using this tool first. Do not fabricate or guess current information. IMPORTANT: The initial search results provide brief summaries that may be outdated, incomplete, or inaccurate, especially for time-sensitive information. When accuracy is critical, use the fetch_url tool to retrieve full webpage content from authoritative sources. CRITICAL RULES: (1) When making a tool call, do NOT generate text like 'I'll check', 'Let me try', or 'Let me search' - just make the tool call directly. (2) After receiving tool results, you MUST immediately continue generating your complete response with the actual answer - do not stop, do not explain what you're going to do, just provide the answer. (3) If you need more information, immediately make another tool call in the same response - do not generate explanatory text first. (4) Your response is incomplete until you provide the actual answer to the user's question. When responding to users, do NOT mention technical details like 'snippets', 'search results', or how you retrieved information. Simply state that you searched the internet or found information online. Keep your response focused on the answer, not the search process.",
        "parameters": {
            "type": "object",
            "properties": {
                "query": {
...
FETCH_URL_TOOL = {
    ...
    "function": {
        "name": "fetch_url",
        "description": "Fetch and extract the actual content from a webpage URL. Use this tool when you need accurate, up-to-date information from a specific webpage, especially after receiving search results. This tool retrieves the full text content from the webpage, which is more reliable than initial search summaries. You should use this tool when: (1) Initial search summaries seem outdated or incomplete, (2) You need detailed information from a specific source, (3) The information is time-sensitive (weather, news, stocks, etc.), (4) Multiple search results conflict and you need to verify the actual content. The tool will extract the main text content from the webpage, excluding navigation, ads, and other non-content elements. CRITICAL: When responding to users, do NOT mention technical details about fetching URLs or how you retrieved information. Simply provide the answer based on the information you found.",
        "parameters": {
            "type": "object",
            "properties": {
                "url": {
...
def call_openrouter_streaming(
    ...
    if not conversation_history:
        ...
        if enable_web_search:
            ...
            system_content += "\n\nYou have access to two web tools:\n1. search_web - Search the internet for current information\n2. fetch_url - Fetch actual content from a specific webpage URL\n\nCRITICAL: For any question requiring real-time, current, or up-to-date information (such as current weather, recent news, current events, live data, stock prices, sports scores, or any information that changes over time), you MUST use the search_web tool first. You are NOT allowed to provide information about current events, weather, news, or any time-sensitive data without first using the search_web tool. Do not fabricate, guess, or cite sources you have not actually checked through the search tool. If you have not used the search_web tool, you cannot provide current information - you must use the tool first.\n\nIMPORTANT: The initial search results provide brief summaries that may be OUTDATED, INCOMPLETE, or INACCURATE, especially for time-sensitive information like weather forecasts, stock prices, sports scores, or breaking news. These summaries may not reflect the current state of the actual webpage.\n\nWHEN TO USE fetch_url: After receiving search results, use the fetch_url tool to get accurate, up-to-date content from specific webpages when:\n- Initial search summaries seem outdated, incomplete, or inaccurate\n- You need detailed information from a specific authoritative source\n- The information is time-sensitive (weather, news, stocks, etc.)\n- Multiple search results conflict and you need to verify actual content\n- You need more detailed information than what the initial summaries provide\n\nThe fetch_url tool retrieves the actual webpage content, which is much more reliable than initial summaries. Always prefer using fetch_url for critical or time-sensitive information rather than relying solely on initial summaries.\n\n🚨 CRITICAL RULES FOR TOOL CALLS:\n1. NEVER generate explanatory text before making a tool call (e.g., 'Let me try...', 'I'll check...', 'Let me search...'). Just make the tool call directly.\n2. NEVER generate explanatory text after receiving tool results without immediately following through. If you need more information, immediately make another tool call - do not explain what you're going to do.\n3. After ANY tool call completes, you MUST immediately continue generating your complete response with the actual answer. Do not stop, do not explain what you're doing - just provide the answer.\n4. If you say anything like 'Let me try X' or 'I'll check Y', you MUST immediately make that tool call in the same response, and then continue with the answer.\n5. Your response is NOT complete until you have provided the actual answer to the user's question. Tool calls are just a step to get information - you must always finish with the answer.\n\nCRITICAL USER-FACING RESPONSE GUIDELINES: When responding to users, do NOT mention:\n- Technical terms like 'snippets', 'search results', 'fetch_url', 'tool calls', or any implementation details\n- How you searched or retrieved information (e.g., 'I searched the web and found...' is acceptable, but avoid details about tools or methods)\n- The search process or any intermediate steps\n- What you're going to do ('Let me try...', 'I'll check...')\n\nInstead, simply provide the answer naturally. If you need to reference that you found information online, you can say 'I searched the internet' or 'based on current information', but keep it brief and user-friendly. Focus on answering the question, not explaining your process."
                        ...
                        if tool_call["function"]["name"] == "search_web":
                            try:
                                ...
                                if search_query:
                                    ...
                                    results_text += "⚠️ IMPORTANT: The summaries below are brief previews provided by search engines and may be OUTDATED, INCOMPLETE, or INACCURATE, especially for time-sensitive information (weather, stocks, sports, news). Treat these as preliminary summaries only. For accurate current information, use fetch_url to retrieve full webpage content from authoritative sources.\n\n"
                                    for i, result in enumerate(search_results, 1):
                                        results_text += f"{i}. {result.title}\n"
                                        results_text += f"   URL: {result.url}\n"
                                        results_text += f"   Summary (may be outdated): {result.snippet}\n\n"
                                    
                                    # Add instruction to help model know when to stop and provide answer
                                    results_text += "\n\n💡 CRITICAL INSTRUCTION: You now have search results. **DO NOT use these initial summaries as your final answer source** - they are unreliable. For accurate responses, especially for time-sensitive information (weather, stocks, news, current events), you MUST use the fetch_url tool to retrieve actual webpage content. Only provide an answer directly from these summaries if: (1) The query is NOT time-sensitive, (2) Multiple summaries from authoritative sources agree, AND (3) The information is general knowledge that doesn't change frequently. When in doubt, use fetch_url for accuracy.\n\n🚨 MANDATORY - YOU MUST CONTINUE NOW: You MUST immediately continue generating your complete response. Do NOT stop. Do NOT say 'Let me try...' or 'I'll check...' - if you need more information, immediately make the tool call AND then continue with the answer. Your response is incomplete until you provide the actual answer to the user's question. Start providing the answer NOW.\n\n⚠️ USER RESPONSE GUIDELINE: When responding to the user, do NOT mention 'snippets', 'summaries', 'search results', 'fetch_url', or any technical search details. Do NOT say what you're going to do ('Let me try...'). Simply provide the answer naturally. If referencing that you found information online, you can say 'I searched the internet' or 'based on current information', but keep it brief. Focus on the answer, not the search process."
                            ...
                            try:
                                ...
                                if url:
                                    ...
                                    content_text = f"Content fetched from {url}:\n\n{url_content}\n\n[End of content from {url}]\n\n💡 CRITICAL INSTRUCTION: You now have webpage content above. **STOP making tool calls immediately and provide your answer now.** You have sufficient information - do not fetch more URLs or make more searches. Answer the user's question based on the content you have.\n\n🚨 MANDATORY - YOU MUST CONTINUE NOW: You MUST immediately continue generating your complete response. Do NOT stop. Do NOT say 'Let me try another source...' or 'I'll check...' - you have the information, provide the answer NOW. If the content doesn't have what you need, use what you have to provide the best answer possible, or immediately make ONE more tool call if absolutely necessary, but then IMMEDIATELY continue with the answer. Your response is incomplete until you provide the actual answer to the user's question. Start providing the answer NOW."
