    ...
    for attempt in range(config.max_retries):
        try:
            ...
            if config.retry_on_network_error and attempt < config.max_retries - 1:
                ...
                await asyncio.sleep(wait_time)
                continue
            logger.error(f"{config.provider_name} request error (attempt {attempt + 1}/{config.max_retries}): {e}")
            raise Exception(f"Failed to connect to {config.provider_name} API: {str(e)}")
            
        except Exception as e:
            # Don't retry unexpected errors
            logger.error(f"Unexpected error in {config.provider_name} (attempt {attempt + 1}/{config.max_retries}): {e}")
            raise
