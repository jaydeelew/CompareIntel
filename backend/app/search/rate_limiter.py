...
class SearchResultCache:
    ...
    def get(self, provider_name: str, query: str) -> Any | None:
        """
        ...
        with self._lock:
            if query_hash in self._cache:
                ...
                if now < expiry_time:
                    logger.debug(
                        f"Cache hit for query: {query[:50]}... (provider: {provider_name})"
                    )
                    return results
                # Expired, remove it
    ...
    def _cleanup_expired(self) -> None:
        ...
        now = time.time()
        expired_keys = [key for key, (expiry_time, _) in self._cache.items() if now >= expiry_time]
    ...
    try:
        ...
        for provider_name, config in config_dict.items():
            provider_configs[provider_name] = ProviderRateLimitConfig(
                max_requests_per_minute=config.get(
                ...
                delay_between_requests=config.get(
...
_global_rate_limiter: SearchRateLimiter | None = None
