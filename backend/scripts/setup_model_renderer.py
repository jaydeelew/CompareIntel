...
from app.model_runner import call_openrouter
...
class ModelRendererSetup:
    ...
    def log_progress(self, stage: str, message: str, progress: float | None = None):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                response = await loop.run_in_executor(
                    None, call_openrouter, prompt_text, self.model_id, "standard", None, False
                )
    ...
    async def collect_responses_async(self) -> dict[str, dict]:
        ...
        for i, (prompt_name, response_data) in enumerate(results):
            ...
            if response_data["success"]:
                ...
                self.log_progress(
                    "collecting", f"Collected {i + 1} of {total_prompts}: {prompt_name}", progress
                )
            else:
                error_preview = response_data.get("error", "Unknown error")[:50]
                self.log(f"  [{i + 1}/{total_prompts}] ✗ {prompt_name} ({error_preview})")
                self.log_progress(
                    "collecting", f"Failed {i + 1} of {total_prompts}: {prompt_name}", progress
                )

        self.log_progress(
            "collecting",
            f"Completed collecting {self.stats['successful']} successful responses",
            100.0,
        )
        return responses
    ...
    def analyze_responses(self, responses: dict[str, dict]) -> dict[str, Any]:
        ...
        self.log_progress(
            "analyzing",
            f"Analysis complete. Found {len(all_delimiters['display'])} display and {len(all_delimiters['inline'])} inline delimiter types",
            100.0,
        )
    ...
    def generate_config(self, analysis: dict[str, Any]) -> dict[str, Any]:
        ...
        self.log_progress(
            "generating", f"Generating renderer configuration for {self.model_id}...", 0.0
        )
    ...
    def _create_delimiter_pattern(
        self, delimiter_type: str, priority: int
    ) -> dict[str, Any] | None:
    ...
    def run(self) -> dict[str, Any]:
        ...
        # Step 4: Save config
        self.log_progress("saving", "Saving renderer configuration to file...", 0.0)
        config_path = self.save_config(config)
        self.log_progress("saving", "Configuration saved successfully", 100.0)

        self.log(f"\n✓ Setup complete! Config saved to {config_path}")
        self.log(
            f"  Successful responses: {self.stats['successful']}/{self.stats['total_requests']}"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Delay between requests (default: 1.0, not used with concurrent collection)",
    )
    parser.add_argument(
        "--max-retries", type=int, default=3, help="Max retries per request (default: 3)"
    )
    parser.add_argument(
        "--concurrency", type=int, default=5, help="Max concurrent requests (default: 5)"
    )
