...
def get_model_token_limits(model_id: str, openrouter_data: dict[str, Any] | None) -> dict[str, int]:
    """
...
def collect_model_data(
    openrouter_data: dict[str, Any] | None,
...
def print_json(models_data: list[dict[str, Any]]):
    ...
    output = {"total_models": len(models_data), "models": models_data}
...
def main():
    ...
    parser.add_argument(
        "--provider", help="Filter by specific provider (e.g., 'OpenAI', 'Anthropic')"
    )
    parser.add_argument(
        "--min-capacity", type=int, help="Filter by minimum input capacity (e.g., 100000)"
    )
    parser.add_argument("--output-file", help="Save output to file (optional)")
    parser.add_argument(
        "--summary", action="store_true", help="Show summary statistics (only for table format)"
    )
    ...
    models_data = collect_model_data(
        openrouter_data, provider_filter=args.provider, min_capacity=args.min_capacity
    )
