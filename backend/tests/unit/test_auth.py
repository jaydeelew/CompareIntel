...
class TestUserRegistration:
    ...
    def test_register_new_user(self, client, db_session):
        ...
        with patch("app.routers.auth.verify_recaptcha", return_value=True):
            ...
            assert "password" not in data.get(
                "user", {}
            )  # Password should not be in user object either
...
class TestEmailVerification:
    ...
    def test_verify_email_success(self, client, db_session):
        ...
        response = client.post("/api/auth/verify-email", json={"token": code})
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
    ...
    def test_verify_email_invalid_code(self, client):
        ...
        response = client.post("/api/auth/verify-email", json={"token": "invalid"})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_verify_email_wrong_code(self, client, db_session):
        ...
        response = client.post("/api/auth/verify-email", json={"token": wrong_code})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_resend_verification_already_verified(self, client, test_user):
        ...
        response = client.post("/api/auth/resend-verification", json={"email": test_user.email})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestUserInfo:
    ...
    def test_delete_account(self, client, db_session):
        ...
        login_response = client.post(
            "/api/auth/login", json={"email": user.email, "password": "test_password_123"}
        )
...
class TestLoginEdgeCases:
    ...
    def test_login_case_sensitive_email(self, client, test_user):
        ...
        response = client.post(
            "/api/auth/login", json={"email": test_user.email.upper(), "password": "secret"}
        )
