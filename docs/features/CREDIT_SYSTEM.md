...
3. **Deduction:** Whole credits deducted once per successful comparison: **monthly pool → overage** (when enabled). Overage is hard-capped in `deduct_credits`; any shortfall beyond the cap is absorbed and logged as `CREDIT_CAP_ABSORBED` — see [Spending cap vs unlimited](#spending-cap-vs-unlimited).
