...
import { ApiError, NetworkError, TimeoutError, CancellationError } from './errors'
import {
  authInterceptor,
...
export class ApiClient {
  ...
  constructor(config: ApiClientConfig) {
    ...
    const cacheEnabled = (config.cache as CacheConfig | false) !== false
    if (cacheEnabled) {
  ...
  private setupDefaultInterceptors(): void {
    ...
    // Error interceptors
    this.errorInterceptors.push(networkErrorInterceptor, loggingErrorInterceptor)
  }
  ...
  private async applyRequestInterceptors(
    ...
    if (this.getToken) {
      ;(finalConfig as RequestConfig & { getToken?: () => string | null }).getToken = this.getToken
    }
  ...
  private async parseResponse<T>(response: Response): Promise<T> {
    ...
    if (contentType?.includes('application/json')) {
      try {
        ...
        throw new ApiError('Failed to parse JSON response', response.status, response.statusText)
      }
    }
  ...
  private async executeWithRetry<T>(
    ...
    const dedupeKey =
      isGetRequest && config.enableCache !== false
  ...
  private async executeRequest<T>(
    ...
    try {
      // Apply request interceptors
      const [finalUrl, finalConfig] = await this.applyRequestInterceptors(url, config)

      // Cleanup timeout on completion
      const cleanup = () => {
        if (finalConfig._timeoutId) {
          clearTimeout(finalConfig._timeoutId)
        }
      }
      ...
      try {
        ...
        const processedResponse = await this.applyResponseInterceptors(response, finalConfig)
      ...
      // Apply error interceptors
      const processedError = await this.applyErrorInterceptors(error as Error, configWithUrl)
  ...
  async stream(
    ...
    try {
      // Apply request interceptors
      const [finalUrl, finalConfig] = await this.applyRequestInterceptors(url, {
        method: 'POST',
        body: data ? JSON.stringify(data) : undefined,
        ...requestConfig,
      })
...
export function createApiClient(config?: Partial<ApiClientConfig>): ApiClient {
  const baseURL =
    config?.baseURL || import.meta.env.VITE_API_URL || import.meta.env.VITE_API_BASE_URL || '/api'
